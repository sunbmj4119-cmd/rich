...
- 미래참조 차단: 재무는 '사용가능일' 기준 as-of join
...
DEFAULT_W = {
    ...
    "logic": {"momentum": 0.35, "profit": 0.30, "stability": 0.20, "growth": 0.15},
...
def main():
    ...
    if has_fin:
        ...
        fin = fin.dropna(subset=["사용가능일"]).sort_values("사용가능일")

        # 매출성장률(전년동기): 같은 분기 1년 전 대비
        ...
        for code, g in df.groupby("종목코드"):
            ...
            if f.empty:
                g["ROE"] = np.nan; g["영업이익률"] = np.nan
                g["부채비율"] = np.nan; g["매출성장률"] = np.nan
            else:
                g = pd.merge_asof(
                    g, f[["사용가능일", "ROE", "영업이익률", "부채비율", "매출성장률"]],
                    left_on="날짜", right_on="사용가능일", direction="backward",
                )
            merged.append(g)
        df = pd.concat(merged)
    else:
        df["ROE"] = np.nan; df["영업이익률"] = np.nan
        df["부채비율"] = np.nan; df["매출성장률"] = np.nan
    ...
    L = w["logic"]
    E = w["emotion"]
    df["논리점수"] = (df["s_mom"] * L["momentum"] + df["s_profit"] * L["profit"]
                  + df["s_stab"] * L["stability"] + df["s_grow"] * L["growth"])
    ...
    cols = ["날짜", "종목코드", "종목명", "종가",
            "논리점수", "감성점수", "종합점수",
            "s_mom", "s_profit", "s_stab", "s_grow", "s_vol", "s_volat"]
    out = out[cols]
