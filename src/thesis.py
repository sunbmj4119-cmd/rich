...
def action_split(ctx, bear, vd, plab, meta=None):
    ...
    if cell:
        ...
        if ret <= STOP and h > 0:
            h *= 0.5
            dead = (plab or {}).get("dead") or {}
            dcell = (dead.get("by_ret") or {}).get("-10%↓")
            why += " ※ 이 칸은 '많이 빠진 자리'라 생존편향이 가장 크다 — "
            if dcell:
                why += (f"실제로 상장폐지된 {dead.get('n_stocks', '?')}종목의 같은 칸은 "
                        f"상승확률 {dcell['up']}%로 {dcell['up'] - cell['up']:+.1f}%p 낮았다. ")
            else:
                why += "끝내 회복 못 하고 사라진 종목은 표본에 없다. "
