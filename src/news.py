...
DART_NEG = {
    ...
    r"자기주식.*해지": (-1, "자사주 매입 신탁 해지 — 사들이던 것을 그만둔다는 뜻"),
}
DART_POS = {
    # '자기주식취득신탁계약해지결정'은 자사주 매입을 **그만두겠다**는 뜻이다.
    # 앞의 '자기주식취득'만 보고 호재로 잡으면 정반대로 읽는다 (첫 CI 실행에서 실제 발생).
    r"자기주식취득(?!.*해지)": (2, "자사주 취득 — 유통 물량이 줄어든다"),
